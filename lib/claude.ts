...
- question: The question text — concise and direct, no unnecessary preamble
...
Generate exactly ${count} additional questions that are similar to the original but explore different angles. Keep questions concise and direct — no unnecessary preamble.${customization ? ` Remember to incorporate the customization: "${customization}"` : ''}${currentEventsContext || ''}
...
const TRANSCRIPT_SNIPPET_MAX_CHARS = 1800;
/** Pull this many chars before/after the matched quote within the segment */
const TRANSCRIPT_SNIPPET_CONTEXT_BEFORE = 520;
const TRANSCRIPT_SNIPPET_CONTEXT_AFTER = 780;
/** When pulling from adjacent segments, cap each side */
const ADJACENT_SEGMENT_CHARS = 580;
...
function buildExpandedTranscriptSnippet(
  ...
  if (snippet.length < 720 && segments.length > 1) {
    if (segmentIndex > 0) {
...
JSON format:
{
  "contentQuality": {
    ...
    "strengths": [{ "text": "Strength", "quote": "Exact contiguous excerpt (~70–200 words when possible: key phrase plus surrounding transcript)" }],
